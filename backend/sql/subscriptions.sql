...
-- to provision the table the backend reads/writes.
-- Column names are snake_case to match backend/app/schemas/subscription.py.
...
-- Row Level Security.
-- The backend uses a single service key server-side, so RLS is left disabled.
