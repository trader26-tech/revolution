...
--   1. On first app launch the backend creates a row in `users` with NO phone —
--      a temporary identity. Its uuid goes back to the app and every task the
--      user picks during onboarding is saved against it IMMEDIATELY (nothing
--      waits for login).
--   2. When the user later enters their phone number, the backend calls
--      claim_user(anon_id, phone):
--        • phone never seen before → the SAME row is claimed (phone filled in,
--          status → 'claimed'). All tasks already point at it. Zero copying.
--        • phone already has an account → the anonymous user's tasks/history/
--          reminders are re-pointed to that existing account atomically, the
--          empty anonymous row is deleted, and the existing account id is
--          returned. The app switches to that id. (Re-install / second device.)
--
-- Three core tables + one auxiliary:
--   users             who (anonymous or claimed)
...
--   reminders         auxiliary outbound-notification queue (push/WhatsApp/call)
--
-- Speed: every hot path is one single-index lookup —
--   home screen        tasks(user_id) WHERE NOT archived
--   calendar           tasks(user_id, due_at)
--   notifier cron      reminders(status, remind_at) WHERE pending
--
...
-- ── 1. users ────────────────────────────────────────────────────────────────
-- One row per person. Born anonymous (phone NULL); claimed on login.
...
create index completions_user_idx on public.task_completions (user_id, completed_at desc);
...
-- Atomic (single transaction): either claims the anonymous row, or merges it
-- into the account that already owns the number. Returns the id the app must
-- use from then on.
...
begin
    ...
    if v_existing is not null and v_existing <> p_anon_id then
        -- The number already has an account (re-install / second device):
        -- move everything the anonymous user made onto it, drop the shell.
    ...
    -- First time this number logs in: claim the anonymous row in place.
    update public.users
       set phone        = p_phone,
           status       = 'claimed',
           display_name = coalesce(p_display_name, display_name),
    ...
    if not found then
        raise exception 'claim_user: unknown user %', p_anon_id;
    end if;
    return p_anon_id;
end;
$$;
