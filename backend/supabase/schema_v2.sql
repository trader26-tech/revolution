...
--   1. On install the APP generates a uuid locally (works offline) and sends it
--      as X-User-Id on every request. The backend materialises the `users` row
--      the first time it sees the id. Every onboarding task saves IMMEDIATELY.
...
create table public.users (
    id            uuid primary key,          -- generated BY THE APP at install
