...
/// Returns the entered key (or "" to clear it when already connected), or null
/// if cancelled. The raw key is never shown back later — once saved, Settings
/// only knows it's "Connected".
Future<String?> showGroqKeySheet(
  BuildContext context, {
  required bool connected,
}) {
  return _open<String>(context, _GroqKeySheet(connected: connected));
}

class _GroqKeySheet extends StatefulWidget {
  const _GroqKeySheet({required this.connected});
  final bool connected;

  @override
  State<_GroqKeySheet> createState() => _GroqKeySheetState();
}

class _GroqKeySheetState extends State<_GroqKeySheet> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              widget.connected
                  ? 'A key is set. Paste a new one to replace it, or remove it '
                      'to go back to the built-in wording.'
                  : 'Paste your Groq API key to let Revora write catchy, '
                      'tailor-made one-liners for your day. Get one free at '
                      'console.groq.com.',
              style: const TextStyle(
            ...
            if (widget.connected) ...[
              ...
              TextButton(
                // Pop "" → the caller clears the key on the server.
